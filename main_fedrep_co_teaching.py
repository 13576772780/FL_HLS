...
from utils.train_utils import get_data_v3, get_model, read_data, get_data, get_data_from_file
from models.Update import LocalUpdate, CoteanchingLocalUpdate
    ...
    for iter in range(args.epochs+1):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                if args.epochs == iter:
                    local = CoteanchingLocalUpdate(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]], idxs=dict_users_train, indd=indd)
                else:
                    local = CoteanchingLocalUpdate(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_tr]], idxs=dict_users_train, indd=indd)
            else:
                if args.epochs == iter:
                    local = CoteanchingLocalUpdate(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_ft])
                else:
                    local = CoteanchingLocalUpdate(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_tr])
