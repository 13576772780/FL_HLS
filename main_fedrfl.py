...
from models.Update import LocalUpdate, LocalUpdatePAC, LocalUpdateRFL
    ...
    for iter in range(args.epochs+1):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                if args.epochs == iter:
                    local = LocalUpdateRFL(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]], idxs=dict_users_train, indd=indd)
                else:
                    local = LocalUpdateRFL(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_tr]], idxs=dict_users_train, indd=indd)
            else:
                if args.epochs == iter:
                    local = LocalUpdateRFL(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_ft])
                else:
                    local = LocalUpdateRFL(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_tr])
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                ...
                w_local, loss, indd, class_center_local, class_num = local.train(net=net_local.to(args.device), f_G=class_center_glob, idx=idx, w_glob_keys=w_glob_keys, lr=args.lr, last=last, concept_matrix_local=concept_matrix[idx])
