...
if __name__ == '__main__':
    ...
    with open('output.txt', 'a') as f:
        ...
        print('# alg: {} , epochs: {}, shard_per_user: {}, limit_local_output: {}, local_rep_ep: {} , local_only: {}, is_concept_shift: {}, dataset: {}  \n'.format(
            args.alg, args.epochs, args.shard_per_user, args.limit_local_output, args.local_rep_ep, args.local_only, args.is_concept_shift, args.dataset))
    ...
    for c in range(args.num_users):
        ...
        if 'femnist' in args.dataset or 'sent140' in args.dataset:
            if args.epochs == iter:
                local = LocalUpdateIncrement(args=args, dataset=dataset_train[list(dataset_train.keys())[c][:args.m_ft]],
                                    idxs=dict_users_train, indd=indd)
            else:
                local = LocalUpdateIncrement(args=args, dataset=dataset_train[list(dataset_train.keys())[c][:args.m_tr]],
                                    idxs=dict_users_train, indd=indd)
        else:
            if args.epochs == iter:
        ...
        for iter in range(args.epochs+1):
            ...
            for ind, idx in enumerate(idxs_users):
                ...
                if 'femnist' in args.dataset or 'sent140' in args.dataset:
                    w_local, loss, indd = local.train(net=net_local.to(args.device),ind=idx, idx=clients[idx], w_glob_keys=w_glob_keys, lr=args.lr, concept_matrix_local=concept_matrix[idx], first=False, isNew=False, local_eps=10, head_eps=5, last=last)
                else:
                    w_local, loss, indd = local.train(net=net_local.to(args.device), idx=idx, w_glob_keys=w_glob_keys, lr=args.lr, concept_matrix_local=concept_matrix[idx], first=False, isNew=False, local_eps=15, head_eps=5, last=last)
            ...
            if iter % args.test_freq==args.test_freq-1 or iter>=args.epochs-10:
                ...
                acc_test, loss_test = test_img_local_all_increment(net_glob, args, dataset_test, dict_users_test,
                                                            w_glob_keys=w_glob_keys, w_locals=w_locals,indd=indd,
                                                                   dataset_train=dataset_train, dict_users_train=dict_users_train, return_all=False, concept_matrix=concept_matrix, num_idxxs=c+1)
                ...
                if args.alg == 'fedavg' or args.alg == 'prox':
                    ...
                    if iter != args.epochs:
                        with open('output.txt', 'a') as f:
                            print('Round {:3d}, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f} \n'.format(
                            iter, loss_avg, loss_test, acc_test))
                    else:
                        with open('output.txt', 'a') as f:
                            print('Final Round, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f} \n'.format(
                            loss_avg, loss_test, acc_test))
                if iter >= args.epochs-10 and iter != args.epochs:
