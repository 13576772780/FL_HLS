...
from models.Update import LocalUpdate, LocalUpdatePAC, LocalUpdatePACPSL, LocalUpdatePACCoTeaching
...
if __name__ == '__main__':
    ...
    print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n')
    print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%FedPAC_co_teaching%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n')
    print(
        '# alg: {} , epochs: {}, shard_per_user: {}, limit_local_output: {}, local_rep_ep: {} , local_only: {}, is_concept_shift: {}, dataset: {}, level_n_system: {} , level_n_lowerb:{}  \n'.format(
            args.alg, args.epochs, args.shard_per_user, args.limit_local_output, args.local_rep_ep, args.local_only,
            args.is_concept_shift, args.dataset,  args.level_n_system, args.level_n_lowerb))
    ...
    for user in range(args.num_users):
        ...
        for key in net_glob.state_dict().keys():
            ...
            w_local_dict2[key] = net_glob2.state_dict()[key]
        w_locals[user] = w_local_dict
        w_locals2[user] = w_local_dict2
    ...
    for iter in range(args.epochs+1):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                if args.epochs == iter:
                    local = LocalUpdatePACCoTeaching(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]], idxs=dict_users_train, indd=indd)
                else:
                    local = LocalUpdatePACCoTeaching(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_tr]], idxs=dict_users_train, indd=indd)
            else:
                if args.epochs == iter:
                    local = LocalUpdatePACCoTeaching(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_ft])
                else:
                    local = LocalUpdatePACCoTeaching(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_tr])


            net_local = copy.deepcopy(net_glob)
            net_local2 = copy.deepcopy(net_glob2)
            w_local = net_local.state_dict()
            w_local2 = net_local2.state_dict()
            if args.alg != 'fedavg' and args.alg != 'prox':
                for k in w_locals[idx].keys():
                    if k not in w_glob_keys:
                        w_local[k] = w_locals[idx][k]
                        w_local2[k] = w_locals2[idx][k]
            net_local.load_state_dict(w_local)
            net_local2.load_state_dict(w_local2)

            last = iter == args.epochs
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                local.filter_data(net=net_local.to(args.device), net2=net_local2.to(args.device),
                                  concept_matrix_local=concept_matrix[idx])
                w_local, loss, indd, class_center_local, class_num = local.train(net=net_local.to(args.device), ind=idx, idx=clients[idx], w_glob_keys=w_glob_keys, lr=args.lr,last=last, concept_matrix_local=concept_matrix[idx],is_teacher=0)
                w_local2, _, _, _, _ = local.train(net=net_local2.to(args.device), ind=idx, idx=clients[idx], w_glob_keys=w_glob_keys, lr=args.lr,last=last, concept_matrix_local=concept_matrix[idx],is_teacher=1)
            else:
                local.filter_data(net=net_local.to(args.device), net2=net_local2.to(args.device), concept_matrix_local = concept_matrix[idx])
                w_local, loss, indd, class_center_local, class_num = local.train(net=net_local.to(args.device), class_center_glob=class_center_glob, idx=idx, w_glob_keys=w_glob_keys, lr=args.lr, last=last, concept_matrix_local=concept_matrix[idx], is_teacher=0)
                w_local2, loss2, indd2, class_center_local2, class_num2 = local.train(net=net_local2.to(args.device),
                                                                                 class_center_glob=class_center_glob,
                                                                                 idx=idx, w_glob_keys=w_glob_keys,
                                                                                 lr=args.lr, last=last,
                                                                                 concept_matrix_local=concept_matrix[
                                                                                     idx],is_teacher=1)
            ...
            if len(w_glob) == 0:
                ...
                w_glob2 = copy.deepcopy(w_local2)
                for k,key in enumerate(net_glob.state_dict().keys()):
                    w_glob[key] = w_glob[key]*lens[idx]
                    w_glob2[key] = w_glob2[key] * lens[idx]
                    w_locals[idx][key] = w_local[key]
                    w_locals2[idx][key] = w_local2[key]
            else:
                for k,key in enumerate(net_glob.state_dict().keys()):
                    if key in w_glob_keys:
                        w_glob[key] += w_local[key]*lens[idx]
                        w_glob2[key] += w_local2[key] * lens[idx]
                    else:
                        w_glob[key] += w_local[key]*lens[idx]
                        w_glob2[key] += w_local2[key] * lens[idx]
                    w_locals[idx][key] = w_local[key]
        ...
        for k in net_glob.state_dict().keys():
            ...
            w_glob2[k] = torch.div(w_glob2[k], total_len)
        ...
        w_local2 = net_glob2.state_dict()
        for k in w_glob.keys():
            w_local[k] = w_glob[k]
            w_local2[k] = w_glob2[k]
        if args.epochs != iter:
