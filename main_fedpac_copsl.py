...
from utils.train_utils import get_data, get_model, read_data, init_class_center, get_data_v2, get_data_v3, \
    get_data_from_file
from models.Update import LocalUpdate, LocalUpdatePAC, LocalUpdatePACPSL, LocalUpdatePACCOPSL
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs+1):
        ...
        #分类客户端
        if len(user_noisy_arr) != 0:
            # p = sorted(user_noisy_arr.items(), key=lambda x: x[1])
            user_noisy_arr_value = [v for v in user_noisy_arr.values()]
            min_err = min(user_noisy_arr_value)
            max_err = max(user_noisy_arr_value)
            for k, v in user_noisy_arr.items():
                if np.abs(v - min_err) <= np.abs(v - max_err):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                if args.epochs == iter:
                    local = LocalUpdatePACCOPSL(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]], idxs=dict_users_train, indd=indd)
                else:
                    local = LocalUpdatePACCOPSL(args=args, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_tr]], idxs=dict_users_train, indd=indd)
            else:
                if args.epochs == iter:
                    local = LocalUpdatePACCOPSL(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_ft], rand_set_all=rand_set_all)
                else:
                    local = LocalUpdatePACCOPSL(args=args, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_tr], rand_set_all=rand_set_all)
            ...
            #上一轮的本地模型
            net_local2.load_state_dict(w_locals[idx])
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                ...
                if iter >= args.init_steps and idx in noisy_client:
                    w_local, loss, indd, class_center_local, class_num, noisy_num = local.train(net=net_local.to(args.device), class_center_glob=class_center_glob, idx=idx, w_glob_keys=w_glob_keys,
                                                                              lr=args.lr, last=last, concept_matrix_local=concept_matrix[idx], iter_num_now=iter, train_iter=iter, local_net = net_local2.to(args.device))
                else:
                    w_local, loss, indd, class_center_local, class_num, noisy_num = local.train_normal(net=net_local.to(args.device), class_center_glob=class_center_glob, idx=idx, w_glob_keys=w_glob_keys, lr=args.lr, last=last, concept_matrix_local=concept_matrix[idx])


            user_noisy_arr[idx] = noisy_num
        ...
        if iter % args.test_freq==args.test_freq-1 or iter>=args.epochs-10:
            ...
            if args.alg == 'fedavg' or args.alg == 'prox' or args.alg == 'fedrep':
                acc_test, loss_test = test_img_local_all(net_glob, args, dataset_test, dict_users_test,
