    ...
    def filter_data(self, net, net2, concept_matrix_local=None):
        ...
        for data_idx in self.idxs:
            if self.args.dataset == "mnist":
                ...
                data_tmp = torch.from_numpy(np.array([self.dataset.data[data_idx].reshape(3, 32, 32)])).to(
                    torch.float32)
            if self.args.is_concept_shift == 1 or self.args.limit_local_output == 1:
                # 通过概念偏移矩阵进行标签概念偏移
                # labels = torch.tensor(concept_matrix_local[labels.numpy()])
...
class LocalUpdatePACCOPSL(object):
    def __init__(self, args, dataset=None, idxs=None, indd=None, rand_set_all = None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        if 'femnist' in args.dataset or 'sent140' in args.dataset:
    ...
    def filter_data(self, net, net2, iter_num,local_eps, concept_matrix_local=None):
    ...
    def train(self, net, w_glob_keys, class_center_glob, last=False, dataset_test=None, ind=-1, idx=-1, lr=0.1, concept_matrix_local=None, iter_num_now = 0, train_iter = 0, local_net = None):
        ...
        optimizer2 = torch.optim.SGD(
            [
                {'params': weight_p2, 'weight_decay': 0.0001},
                {'params': bias_p2, 'weight_decay': 0}
            ],
            lr=lr, momentum=0.5
        )
        ...
        for iter in range(self.args.psl_step):
            ...
            for iter2 in range(local_eps):
                ...
                ## 训练本地模型
                for batch_idx, (images, labels) in enumerate(self.ldr_train2):
                    ...
                    if 'sent140' in self.args.dataset:
                        ...
                        local_net.zero_grad()
                        hidden_train = repackage_hidden(hidden_train)
                        output, hidden_train = local_net(data, hidden_train)
                        loss = self.loss_func(output.t(), torch.max(targets, 1)[1])
                        loss.backward()
                        optimizer.step()
                    else:
                        ...
                        if iter2 >= head_eps and self.args.alg == 'fedrep' and not last:
                            ...
                            #获取特征值
                            if self.args.model == "mlp":
                                local_net.layer_hidden2.register_forward_hook(self.hook)
                            elif self.args.model == "cnn":
                                local_net.fc2.register_forward_hook(self.hook)
                            elif self.args.model == "resnet18":
                                local_net.linear.register_forward_hook(self.hook_input)
