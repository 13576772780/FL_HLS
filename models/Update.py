...
class LocalUpdatePACPSL(object):
    ...
    def filter_by_loss(self, net, concept_matrix_local, iter_num, local_eps):
        ...
        filter_idxs = [sort_distance_tmp[i][0] for i in
                        range(math.floor(self.args.shard_per_user * self.args.nums_per_class * (iter_num+1) * 0.9/ local_eps))]
    ...
    def train(self, net, w_glob_keys, class_center_glob, last=False, dataset_test=None, ind=-1, idx=-1, lr=0.1, concept_matrix_local=None):
        ...
        for iter in range(local_eps):
            ...
            if self.args.filter_alg == 'center_psl':
                self.filter_by_center(net=net, concept_matrix_local=concept_matrix_local, local_class_center=local_class_center, iter_num=iter, local_eps = local_eps)
            else:
